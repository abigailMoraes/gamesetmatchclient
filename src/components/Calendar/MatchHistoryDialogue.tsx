...
function MatchHistoryDialogue(props: IMatchHistoryDialogProps) {
  ...
  const confirmAttendance = () => {
    MatchService.confirmMatchAttendance(1, match.id).then((data) => { console.warn(data); });
  };
  const dropOut = () => {
    MatchService.dropOutOfMatch(1, match.id).then((data) => { console.warn(data); });
  };
  return (
    <Dialog
    ...
    >
      ...
      <DialogActions>
        ...
        {match.results === 'TBD' && match.attendance === 'No'
          ? <Button color="secondary" onClick={confirmAttendance}>Confirm Attendance</Button> : null}
        {match.results === 'TBD' && match.attendance === 'Yes'
          ? <Button color="secondary" onClick={dropOut}>Drop Out</Button> : null}
      </DialogActions>
    </Dialog>
  );
}
